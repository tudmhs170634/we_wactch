...
const UsersView: React.FC<UsersViewProps> = ({ users, onBanUser, onSwitchRole }) => {
  return (
    <div className="overflow-hidden rounded-3xl bg-[#111113] border border-white/5 shadow-sm">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-white/5">
            {users.map((u) => (
              <tr key={u.id} className="group transition-colors hover:bg-white/5/50">
                <td className="px-8 py-6">
                  <div className="flex items-center gap-5">
                    <div className="h-16 w-16 overflow-hidden rounded-2xl border-2 border-white/10 group-hover:border-primary/20 transition-colors relative shadow-sm">
                      <Image 
                ...
                <td className="px-8 py-6 text-center">
                  <span
                    className={`rounded-full px-3 py-1 text-xs font-black uppercase ${
                      u.isBanned ? 'bg-primary/20 text-primary' : 'bg-green-50 text-green-600'
                    }`}
                  >
                ...
                <td className="px-8 py-6 text-right">
                  <div className="flex justify-end gap-3">
                    <button
                      ...
                      className={`flex h-11 w-11 items-center justify-center rounded-xl border border-white/10 transition-all ${
                        u.isBanned
                          ...
                          : 'bg-[#111113] border border-white/5 text-gray-400 hover:border-primary/20 hover:text-primary'
                      }`}
                    >
                    ...
                    <button
                      ...
                      className="flex h-11 w-11 items-center justify-center rounded-xl border border-white/10 bg-[#111113] border border-white/5 text-gray-400 transition-all hover:border-blue-100 hover:text-blue-600"
                    >
                      <Shield size={20} />
                    </button>
                  </div>
