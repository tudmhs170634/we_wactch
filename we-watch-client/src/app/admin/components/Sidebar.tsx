...
export type TabType = 'dashboard' | 'users' | 'rooms' | 'all_movies' | 'movie_queue';
...
const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  const menuItems = [
    ...
    { id: 'all_movies', label: 'Tất cả phim', icon: Film },
    { id: 'movie_queue', label: 'Duyệt phim', icon: Clock },
  ];
  ...
  return (
    <div className="fixed top-0 left-0 h-screen w-64 border-r border-gray-100 bg-white px-6 py-8">
      <div className="mb-12 flex items-center gap-3 px-2">
        ...
        <div>
          <h1 className="text-lg font-black tracking-tight text-gray-900">The Kinetic</h1>
          <p className="text-[10px] font-bold tracking-widest text-gray-400 uppercase">Hệ thống quản trị</p>
        </div>
      </div>
      ...
      <nav className="space-y-1">
        {menuItems.map((item) => (
          ...
          >
            ...
            <item.icon 
              size={20} 
              className={activeTab === item.id ? 'text-red-600' : 'text-gray-400 group-hover:text-gray-900'} 
            />
            {item.label}
          </button>
        ))}
