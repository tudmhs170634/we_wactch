...
import { getRoom, getRoomBySlug, deleteRoom, terminateRoom } from '@/src/services/room';
...
export default function WeWatchRoomPage({
  ...
  const {
    ...
    if (amIHost && stoppedByRole === 'admin') {
      ...
      toast.error(`Phòng của bạn đã bị Admin dừng bởi ${reasonText}`, {
        duration: 6000,
        description: 'Bạn sẽ được chuyển về trang danh sách phòng.',
      });
      setTimeout(() => router.push('/rooms'), 3000);
    } else if (!amIHost) {
      toast.info('Phòng đã kết thúc.', {
        description: stoppedByRole === 'admin'
  ...
  useEffect(() => {
    const fetchRoom = async () => {
      if (!params.slug || isRoomEnded.current) return;
      try {
        setLoading(true);
        let data;
        const isUuid =
            ...
            <div className="flex items-center justify-between border-b border-white/5 p-4">
              <div className="flex items-center gap-4">
                <button
                  ...
                  className={`text-sm font-bold tracking-tighter uppercase transition-colors ${activeTab === 'chat' ? 'text-white' : 'text-white/20'
                    }`}
                >
                  Trò chuyện
                </button>
                ...
                <button
                  ...
                  className={`text-sm font-bold tracking-tighter uppercase transition-colors ${activeTab === 'history' ? 'text-white' : 'text-white/20'
                    }`}
                >
                  Lịch sử
                </button>
                      ...
                      {msg.type === 'status' ? (
                        ...
                        >
                          ...
                          >
                            <div className="flex items-center gap-2">
                              <span
                                className={`text-[10px] font-black tracking-tighter uppercase ${msg.username === room?.host?.username
                                    ? 'text-[#C800DF]'
                                    : 'text-white/40'
                                  }`}
                              >
                            ...
                            <div
                              className={`mt-0.5 px-3 py-1.5 text-[14px] leading-tight ${msg.username === user?.username
